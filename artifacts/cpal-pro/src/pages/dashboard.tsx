...
import {
  ...
  UploadCloud, Settings, Clock, ArrowDownToLine, Trash2
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const [deadlineLabel, setDeadlineLabel] = useState("April 15, 2027");
  ...
  useEffect(() => {
    ...
    const currentCycle = new Date(`${year}-04-15T23:59:59`);
    const deadline = now > currentCycle ? new Date(`${year + 1}-04-15T23:59:59`) : currentCycle;

    const diff = deadline.getTime() - Date.now();
    setDaysLeft(Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24))));
    setDeadlineLabel(deadline.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }));
  }, []);
        ...
        <div className="container mx-auto px-4 md:px-6 pt-10 pb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              ...
              <p className="text-muted-foreground mt-2 font-medium">Welcome back, Jordan. Here&apos;s your filing status.</p>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <Card className="border-border shadow-sm rounded-2xl">
                    <CardContent className="p-6">
                      ...
                      <h3 className="text-xl font-bold text-foreground">{deadlineLabel}</h3>
                      {daysLeft !== null && (
                  ...
                  <Card className="border-border shadow-sm rounded-2xl">
                    <CardContent className="p-6">
                      ...
                      <h3 className="text-base font-bold text-foreground leading-tight">Upload brokerage 1099-B statement</h3>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card className="border-border shadow-sm rounded-2xl overflow-hidden">
                  <CardHeader className="border-b bg-card pb-6">
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-xl font-display">2025 Federal + State Tax Return</CardTitle>
                        <CardDescription className="mt-1 text-sm font-medium">Assigned to: CPA Olivia Bennett</CardDescription>
                      </div>
                  ...
                  <CardContent className="pt-10 pb-10 bg-background">
                    <div className="relative max-w-2xl mx-auto">
                      ...
                      <div className="flex justify-between relative z-10">
                        ...
                        <div className="flex flex-col items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-muted border-2 border-border text-muted flex items-center justify-center border-4 border-background"></div>
              ...
              <Card className="border-border shadow-sm rounded-2xl">
                <CardHeader className="flex flex-row items-center justify-between border-b pb-6">
                  <div>
                    ...
                    <CardDescription>Files shared for your return.</CardDescription>
                  </div>
                ...
                <CardContent className="p-0">
                  <div className="divide-y border-border">
                    {[
                      { name: "W2_2025.pdf", type: "W-2", date: "Oct 12", status: "Verified", color: "text-primary bg-primary/10" },
                      { name: "Checking_Statement_Q4.pdf", type: "Bank Record", date: "Oct 12", status: "Verified", color: "text-primary bg-primary/10" },
                      { name: "1099B_Brokerage.pdf", type: "1099-B", date: "Oct 12", status: "Pending", color: "text-accent bg-accent/10" }
                    ].map((doc, i) => (
              ...
              <Card className="border-border shadow-sm rounded-2xl flex flex-col h-[600px]">
                <CardHeader className="border-b py-4 px-6 bg-card">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-accent/20 flex items-center justify-center text-accent font-bold">OB</div>
                    <div>
                      <CardTitle className="text-base">CPA Olivia Bennett</CardTitle>
                ...
                <CardContent className="flex-1 p-6 overflow-y-auto space-y-6 bg-muted/10">
                  ...
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center text-accent font-bold text-xs shrink-0 mt-1">OB</div>
                    <div className="bg-card border shadow-sm p-4 rounded-2xl rounded-tl-none max-w-[80%]">
                      <p className="text-sm text-foreground">Hi Jordan, I&apos;m reviewing your return now. Could you upload your 1099-B brokerage statement for tax year 2025?</p>
                    </div>
                  </div>

                  <div className="flex gap-3 justify-end">
                    <div className="bg-primary text-white shadow-sm p-4 rounded-2xl rounded-tr-none max-w-[80%]">
                      <p className="text-sm">Absolutely, I&apos;ll upload it this afternoon.</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center text-accent font-bold text-xs shrink-0 mt-1">OB</div>
                    <div className="bg-card border shadow-sm p-4 rounded-2xl rounded-tl-none max-w-[80%]">
                      <p className="text-sm text-foreground">Perfect, thank you. Once it&apos;s uploaded, I&apos;ll prepare the draft return for your approval.</p>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="border-border shadow-sm rounded-2xl bg-navy text-white overflow-hidden relative">
              ...
              <CardContent className="p-8 relative z-10">
                <div className="flex flex-col items-center text-center">
                  ...
                  <h4 className="font-bold text-xl font-display text-white mb-1">CPA Olivia Bennett</h4>
                  <p className="text-sm font-medium text-white/70 mb-2">Specialty: Founders & Equity Compensation</p>
